...
exports.checkout = async (req, res) => {
  try {
    ...
    for (const item of userCart.products) {
      ...
      if (product.farmacia._id.toString() !== farmaciaId.toString()) {
        // If any item is from a different pharmacy, redirect to cart with error message
        req.flash('error_msg', 'Você só pode comprar produtos de uma farmácia por vez.');
    ...
    if (req.body.paymentMethod === "Pagamento na entrega") {
     ...
     if (req.body.paymentMethod === "PayPal") {
      // Verifica se orderDetails.finalPrice é um número antes de tentar formatá-lo
      let finalPrice = parseFloat(orderDetails.finalPrice);
      
      if (isNaN(finalPrice)) {
        // Trate o caso em que orderDetails.finalPrice não é um número válido
        finalPrice = 0; // Ou outra ação adequada ao seu caso
      } else {
        ...
        transactions: [
          {
              ...
              items: [
                {
                  ...
                  price: finalPrice, // Use o valor formatado aqui
                  currency: "USD",
                  quantity: 1,
                },
              ],
            ...
            amount: {
              currency: "USD",
              total: finalPrice, // Use o valor formatado aqui também
            },
...
exports.result = async (req, res) => {
  try {
    ...
    if (req.session.transactionID) {
      ...
      // Ensure orderDetails has the farmacia property
      const orderDetails = new orderCLTN({
        ...req.session.orderDetails,
        farmacia: req.session.orderDetails.farmacia, // Add the pharmacy ID
      });
