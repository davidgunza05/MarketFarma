...
exports.ourCollection = async (req, res) => {
  try { 
    ...
    if (!listing) {
      listing = await productCLTN.find({ listed: true }).populate('farmacia');
    }
    const categories = await categoryCLTN.find({});

    const sort = req.query.sort || 'default'; // Obtém o parâmetro de ordenação da query string 
    if (sort === 'crescente') {
      listing = await productCLTN.find().populate('farmacia').sort({ price: 1 }).exec(); // Ordena por preço ascendente
      //listing = await productCLTN.find().sort({ reviews: 1 }).exec(); // Ordena por preço ascendente
    } else if (sort === 'decrescente') {
      listing = await productCLTN.find().populate('farmacia').sort({ price: -1 }).exec(); // Ordena por nome em ordem alfabética
    } else if (sort === 'A_a_Z') {
      listing = await productCLTN.find().populate('farmacia').sort({ name: 1 }).exec(); // Ordena por nome em ordem alfabética
    } else if (sort === 'Z_a_A') {
      listing = await productCLTN.find().populate('farmacia').sort({ name: -1 }).exec(); // Ordena por nome em ordem alfabética
    } else {
      listing = await productCLTN.find().populate('farmacia').exec(); // Mantém a ordem padrão
    }
...
exports.sortBy = async (req, res) => {
  try {
    if (!req.session.listing) {
      req.session.listing = await productCLTN.find({ listed: true }).populate('farmacia');
    }
    let listing = req.session.listing;
