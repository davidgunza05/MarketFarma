...
exports.viewAll = async (req, res) => {
  try {
    ...
    const allProducts = await Produto.find({listed: true}).populate('farmacia').sort({ _id: -1 })
    const farmaciaId = allProducts.farmacia;
    let men = [];
    let women = []; 
    ...
    const sold = await Produto.find({listed: true}).populate('farmacia').sort({ views: -1 });
