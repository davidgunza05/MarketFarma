...
router.post('/farmacia/activate/:id', (req, res) => {
  Farmacia.findById(req.params.id).then(farmacia => {
    if (!farmacia) {
      req.flash('error_msg', 'Farmácia não encontrada');
      return res.redirect('/admin/farmacias');
    }
    farmacia.isActive = !farmacia.isActive;
    const status = farmacia.isActive ? 'ativada' : 'desativada';
    
    farmacia.save().then(() => {
      req.flash('success_msg', `Farmácia ${status} com sucesso`);
      res.redirect('/admin/farmacias');
    }).catch(err => {
      req.flash('error_msg', 'Erro ao salvar as alterações');
      res.redirect('/admin/farmacias');
    });
  }).catch(err => {
    req.flash('error_msg', 'Erro ao encontrar a farmácia');
    res.redirect('/admin/farmacias');
  });
});
...
router.post('/farmacias/delete/:id', async (req, res) => {
  try {
    ...
    console.error(err);
    req.flash('error_msg', 'Erro ao deletar farmácia e produtos');
    res.redirect('/admin/farmacias');
  } 
});
